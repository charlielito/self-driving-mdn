    ...
    def load_row(row):
        ...
        if mode == "train":
            sample_weight = tf.cond(
                tf.equal(row["original_steering"], 0),
                lambda: params.percent_zero,
                lambda: 1.0,
            )
...
def get_model(params, components=None) -> tf.keras.Model:
    ...
    if components is not None:
        ...
        alpha_v = tf.keras.layers.Dense(
            components, activation="softmax", name="alphas"
        )(x)
        mu_v = tf.keras.layers.Dense(components, name="mus")(x)
        sigma_v = nnelu(tf.keras.layers.Dense(components, name="sigmas")(x))

        x = tf.keras.layers.Concatenate(name="pvec")([alpha_v, mu_v, sigma_v])

    else:
        x = tf.keras.layers.Dense(1, name="steering", use_bias=False)(x)

    model = tf.keras.Model(inputs=inputs, outputs=x, name="nvidia_net_mdn")
